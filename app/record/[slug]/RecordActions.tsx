...
export function RecordActions({slug,url,summary,credential}:{slug:string;url:string;summary:string;credential:{name:string;issuer:string;issueDate:string;credentialId:string;digest:string}}){
  ...
  return <>
    ...
    <section className="linkedin-shell">
      ...
      <div><span>SHA-256 DIGEST</span><b>{credential.digest}</b><button onClick={()=>void copy(credential.digest,"CREDENTIAL DIGEST COPIED")}>COPY</button></div>
      <div><span>CREDENTIAL URL</span><b>{url}</b><button onClick={()=>void copy(url,"CREDENTIAL URL COPIED")}>COPY</button></div>
      <p>The digest verifies the immutable simulation record payload, not personal identity, accreditation, or licensure. The LinkedIn handoff remains dormant until the DELENDA.QUEST LinkedIn Page supplies the official issuer identity.</p>
    </section>
  </>;
}
