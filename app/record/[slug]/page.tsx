...
export default async function CampaignRecordPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="public-record">
    ...
    <article className="record-certificate">
      ...
      <section className="record-scoreboard"><div><span>CAMPAIGN SCORE</span><b>{record.campaignScore.toLocaleString()}</b><small>RANK {record.campaignRank} / {record.cohortSize}</small></div><div><span>UBERSCORE EARNED</span><b>+{record.uberscoreEarned.toLocaleString()}</b><small>{record.friendCount} FRIENDS // ×{record.friendMultiplier.toFixed(2)}</small></div><div><span>CUMULATIVE UBERSCORE</span><b>{record.uberscore.toLocaleString()}</b><small>GLOBAL RANK {record.globalRank} / {record.commanderCount}</small></div></section>
