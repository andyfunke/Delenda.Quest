...
import { authenticatedSignInPath, getAuthenticatedUser } from "../../auth";
...
export async function POST(request:Request){
  const user=await getAuthenticatedUser();if(!user)return NextResponse.json({error:"Sign in before inviting a friend."},{status:401});
...
export async function DELETE(request:Request){
  const user=await getAuthenticatedUser();if(!user)return NextResponse.json({error:"Sign in before changing friends."},{status:401});
...
export async function PATCH(request:Request){
  const user=await getAuthenticatedUser();if(!user)return NextResponse.json({error:"Sign in before changing account settings."},{status:401});
