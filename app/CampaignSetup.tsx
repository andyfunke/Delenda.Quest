...
export function CampaignSetup({current,hasSave,seedOverride,onStart,onResume,onClose}:{
  ...
  const[config,setConfig]=useState<CampaignConfig>({
    seed:sanitizeSeed(seedOverride??current.campaignSeed),
  ...
  return <div className="campaign-setup os-window" role="dialog" aria-modal="true" aria-labelledby="campaign-generator-title">
    ...
    <header>
      <span className="eyebrow">CAMPAIGN GENERATOR // THE SAME SEED REPRODUCES THE SAME WAR</span>
      <h2 id="campaign-generator-title">Select the state that will be spent.</h2>
      <p>Theater chooses the situation corpus. State archetype changes your opening ledger. Enemy system changes how the opposition learns and prosecutes the war. <a className="campaign-help-link" href="?wiki=campaign-seed" target="_blank" rel="noreferrer">CAMPAIGN GENERATION RULE ↗</a></p>
    </header>
    <div className="campaign-generator-layout">
      ...
      <article className="campaign-config-inspector">
        ...
        <section className="campaign-seed-control">
          ...
          <input id="campaign-seed" inputMode="numeric" min="1" max="2147483647" type="number" value={config.seed} onChange={e=>set("seed",sanitizeSeed(Number(e.target.value)))}/>
          <button onClick={()=>set("seed",randomSeed())}>RANDOMIZE</button>
        </section>
      </article>
    </div>
