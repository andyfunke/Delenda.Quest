...
export function CampaignSituationPanel({s,compact=false}:{s:GameState;compact?:boolean}){
  ...
  return <section className={`campaign-substrate ${compact?"compact":""}`}>
    ...
    <div className="substrate-ledger">
      ...
      <ManualLink id="resolution-ticket" label={situation.resolutionTicket.split(":").at(-1)?.toUpperCase()??"SEALED"} detail="Stored daily record. Every preview and the final resolution use the same record; reopening the screen cannot change it."/>
    ...
    <div className="substrate-body">
      ...
      <aside>
        ...
        </>:<p>Sector record unavailable. Theater headquarters will restore the baseline at the next day boundary.</p>}
        <h3>ACTIVE FACTS</h3>
