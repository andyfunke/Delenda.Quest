...
export const CAMPAIGN_PHASES:CampaignPhase[]=[
  ...
  {id:"compression",label:"Operational Compression",days:[6,12],brief:"Routes, reserves, and timetables have narrowed. Every useful asset is now close enough to the front to be consumed by it.",quote:"Concentration begins as efficiency and ends as a target description.",exact:["Supply use ×1.08","Enemy front pressure +0.10 km","Industrial output ×0.98","Combat casualty factor ×1.04"],modifiers:{supplyUse:1.08,enemyPressure:.1,productionOutput:.98,casualty:1.04}},
...
export const CAMPAIGN_EVENTS:CampaignEvent[]=[
  ...
  {id:"national-reserve",label:"The National Reserve Is Released",category:"TERMINAL OPERATIONS",phases:["terminal"],brief:"The formation preserved for continuity of government has been transferred to operational command. Continuity will now be attempted by winning.",report:"The national reserve increased pressure and confidence at exceptional casualty exposure.",quote:"A final reserve is the moment preservation admits it has failed.",exact:["Friendly front pressure +0.35 km","Execution Confidence +5%","Combat casualty factor ×1.22"],risk:["No equivalent reserve remains after resolution"],modifiers:{friendlyPressure:.35,confidence:.05,casualty:1.22}},
  ...
  {id:"shell-famine",label:"The Shell Famine Is Now a Tactical Condition",category:"REACTIVE CRISIS",phases:["contact","compression","exhaustion","terminal"],trigger:"Munitions coverage below 2 days",brief:"Fire plans are being rewritten around rounds that do not exist. Batteries retain targets and lose permission to engage them.",report:"Critical munitions coverage reduced execution and pressure while increasing casualties.",quote:"A gun without shells remains useful to the report that counts guns.",exact:["Execution Confidence -6%","Friendly front pressure -0.25 km","Combat casualty factor ×1.18","Supply use ×0.82"],risk:["Continued shortage can trigger this crisis again after one intervening day"],modifiers:{confidence:-.06,friendlyPressure:-.25,casualty:1.18,supplyUse:.82}},
...
export const FAMILIES: Family[] = [
  ...
  {id:"treaties",module:"diplomacy",category:"Treaty Architecture",label:"Bind Foreign Obligations",brief:"Write today's access into tomorrow's automatic duty, then discover which party understood the verbs differently.",lock:4,choices:[
  ...
  {id:"sanctions",module:"diplomacy",category:"Economic Warfare",label:"Administer Sanctions",brief:"Convert market access into coercion while counting which dependencies point inward.",lock:3,choices:[
  ...
  {id:"alliance-obligations",module:"diplomacy",category:"Alliance Management",label:"Service Alliance Obligations",brief:"Decide whether the alliance is a source of materiel, a claimant on it, or merely a future accusation.",lock:2,choices:[
...
const DIPLOMACY_DURATION:Record<string,number>={supply:6,statecraft:4,treaties:8,sanctions:6,"alliance-obligations":4};
...
export const maneuverChance = (s: GameState, m: Maneuver, director:CampaignDirector=directorForState(s)) => {
  ...
  const doctrine=(m.id==="interdict"&&s.unlocked.includes("long-range")?.09:0)+(m.id==="breach"&&s.unlocked.includes("forced-passage")?.12:0)+(m.id==="network"&&s.unlocked.includes("relay-discipline")?.06:0);const adaptation=-Math.min(.12,(s.adversary?.adaptation[m.id]??0)*.015);
  return Math.max(.05, Math.min(.95, m.success + intelligence + readiness + equipment + shortages + fieldProof + doctrine + adaptation + director.modifiers.confidence));
};
