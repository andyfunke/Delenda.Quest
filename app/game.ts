...
export const maneuverChance = (s: GameState, m: Maneuver) => {
  ...
  return Math.max(.05, Math.min(.95, m.success + intelligence + readiness + equipment + shortages + fieldProof + doctrine + adaptation));
};
...
export const resolve = (state: GameState) => {
  ...
  s.day+=1; s.actions=DAILY_ORDERS; s.maneuver=null; s.reports.unshift({ day:s.day, title:resultTitle, body:`${losses.toLocaleString()} soldiers were lost. The front moved ${move>=0?"+":""}${move.toFixed(1)} km.${operationsText}${adversaryText}${diplomacyText}${domesticText}${forceText}${productionText}${desertText}${doctrineText}`, tone:move>.6?"good":move<-.4?"bad":"warn", epigraph:maneuver?`Experience is waste until it is made transmissible.`:`The map is never empty. It contains roads not yet cut, fields not yet denied, and men not yet counted.` });
