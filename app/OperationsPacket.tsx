...
export function OperationsPacket({s,m}:{s:GameState;m:Maneuver}){
  ...
  return <section className="operations-packet">
    ...
    <div className="operations-summary">
      ...
      <InspectCell id="effective-committed-force" label="EFFECTIVE DEPLOYED" value={fmtStrategic(operation.effectiveCommitted)} note="after all battlefield conversion" details={[{label:"FRIENDLY DEPLOYED",value:fmtStrategic(operation.committed)},{label:"READINESS",value:`× ${operation.readinessFactor.toFixed(2)}`},{label:"EQUIPMENT",value:`× ${operation.equipmentFactor.toFixed(2)}`},{label:"TERRAIN",value:`× ${operation.terrainFactor.toFixed(2)}`},{label:"GROUND",value:`× ${operation.groundFactor.toFixed(2)}`},{label:"NETWORK",value:`× ${operation.networkFactor.toFixed(2)}`},{label:"SUPPLY",value:`× ${operation.supplyFactor.toFixed(2)}`},{label:"INTELLIGENCE",value:`× ${operation.intelligenceFactor.toFixed(2)}`},{label:"FRONTAGE",value:`${Math.round(operation.frontageSaturation*100)}% of ${fmtStrategic(operation.frontageDemand)} useful capacity; congestion begins above 135%`},{label:"RESULT",value:fmtStrategic(operation.effectiveCommitted)}]}/>
      <InspectCell id="enemy-forward-deployment" label="ASSESSED ENEMY" value={fmtStrategic(adversary.deployedEstimate)} note={`forward deployed // ${fmtStrategic(adversary.deployedLow)}–${fmtStrategic(adversary.deployedHigh)}`} details={[{label:"FORWARD DEPLOYED ESTIMATE",value:fmtStrategic(adversary.deployedEstimate)},{label:"FORWARD DEPLOYMENT BAND",value:`${fmtStrategic(adversary.deployedLow)}–${fmtStrategic(adversary.deployedHigh)}`},{label:"TOTAL ASSESSED FIELD FORCE",value:fmtStrategic(adversary.estimatedForce)},{label:"TOTAL FIELD FORCE BAND",value:`${fmtStrategic(adversary.estimateLow)}–${fmtStrategic(adversary.estimateHigh)}`},{label:"FORWARD COMMITMENT",value:`${Math.round(adversary.deploymentShare*100)}% under ${adversary.posture}`},{label:"READINESS",value:`${s.adversary.readiness.toFixed(0)}%`},{label:"EQUIPMENT",value:`${s.adversary.equipment.toFixed(0)}%`},{label:"REINFORCEMENT AT RESOLUTION",value:`+${fmtStrategic(adversary.reinforcement)}`},{label:"ASSESSMENT BASIS",value:"Assessed total × posture commitment; uncertainty contracts as Intelligence rises"}]}/>
      <InspectCell id="force-ratio" label="LOCAL FORCE RATIO" value={`${operation.forceRatio.toFixed(2)} : 1`} note={`${fmtStrategic(operation.friendlyPower)} vs ${fmtStrategic(operation.enemyPower)} combat power`} details={[{label:"FRIENDLY POWER",value:fmtStrategic(operation.friendlyPower)},{label:"ENEMY POWER",value:fmtStrategic(operation.enemyPower)},{label:"ENEMY FORWARD DEPLOYED",value:fmtStrategic(operation.enemyCommitted)},{label:"FORMULA",value:"friendly local combat power ÷ assessed enemy local combat power"},{label:"PRESSURE CONTRIBUTION",value:`${operation.forceRatioPressure>=0?"+":""}${operation.forceRatioPressure.toFixed(2)}`}]}/>
    </div>
