...
function Concept({id,children}:{id:string;children:React.ReactNode}){const c=CONCEPTS[id];if(!c)return <span>{children}</span>;return <span className="term" role="button" tabIndex={0} onClick={()=>openWikiApplet(id)}>{children}<span className="term-tip"><b>{c.label}</b><span>{c.definition}</span>{c.normal&&<em>NORMAL // {c.normal}</em>}<strong>CONSEQUENCE // {c.consequence}</strong><button onClick={e=>{e.stopPropagation();openWikiApplet(id)}}>OPEN WIKI APPLET</button><a href={`?wiki=${id}`} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()}>OPEN EXTERNALLY ↗</a></span></span>}
...
export function OperationsPacket({s,m}:{s:GameState;m:Maneuver}){
  ...
  return <section className="operations-packet">
    ...
    <div className="outcome-band-matrix">{contract.bands.map(({id,label,margin})=>{const branch=projections[id];const fact=id==="clean"?contract.aftermath?.clean??contract.aftermath?.success:id==="executed"?contract.aftermath?.success:contract.aftermath?.failure;return <button onClick={()=>openWikiApplet("outcome-margin")} key={id} className={`band-${id}`} title={`${label}: confidence minus the stored resolution roll falls in the ${margin} band.`}><small>{label}</small><b>{branch.groundMovement>=0?"+":""}{branch.groundMovement.toFixed(2)} KM</b><span>−{fmt(branch.friendlyLosses,true)} friendly // {fmt(branch.enemyLosses,true)} enemy est.</span><em>{fact?.label??"No persistent fact"}</em></button>})}</div>
    <details><summary>SHOW PRESSURE CALCULUS AND EVIDENCE</summary><div className="pressure-ledger">{[["BASE + TEMPO",o.basePressure],["MANEUVER RANGE",`${m.failurePressure>=0?"+":""}${m.failurePressure.toFixed(2)} to ${m.successPressure>=0?"+":""}${m.successPressure.toFixed(2)}`],["FORCE RATIO",o.forceRatioPressure],["INTELLIGENCE",o.intelligencePressure],["SHORTAGES",o.shortagePressure],[`STRATEGIC CONDITION // ${director.event.label.toUpperCase()}`,conditionPressure]].map(([label,value])=><div key={String(label)}><span>{label}</span><b>{typeof value==="number"?`${value>=0?"+":""}${value.toFixed(2)} KM`:`${value} KM`}</b></div>)}</div><ul><li>Strategic condition: {director.event.label} // friendly pressure {director.modifiers.friendlyPressure>=0?"+":""}{director.modifiers.friendlyPressure.toFixed(2)} // enemy pressure {director.modifiers.enemyPressure>=0?"+":""}{director.modifiers.enemyPressure.toFixed(2)}</li>{o.evidence.filter(x=>!x.startsWith("Resolution roll")&&!x.includes("ground movement")&&!x.includes("enemy losses")).map(x=><li key={x}>{x}</li>)}</ul></details>
  </section>;
}
