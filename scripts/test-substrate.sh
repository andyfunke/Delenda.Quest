...
OPERATIONAL_BUNDLE="$(mktemp "${TEMP_ROOT}/delenda-ava-operational.XXXXXX.mjs")"
...
trap 'rm -f "$GATE_BUNDLE" "$DOCKET_BUNDLE" "$SERVICES_BUNDLE" "$PARSER_BUNDLE" "$AVA_BUNDLE" "$INDEX_BUNDLE" "$LLM_BUNDLE" "$MCP_BUNDLE" "$TERMINAL_BUNDLE" "$SSH_BUNDLE" "$SSH_GATEWAY_BUNDLE" "$NEXUS_BUNDLE" "$COMPILER_BUNDLE" "$GAME_BUNDLE" "$COGNITIVE_BUNDLE" "$COGNITIVE_NEXUS_BUNDLE" "$AVA_RUNTIME_BUNDLE" "$AVA_PROJECTION_BUNDLE" "$CONTEXTUAL_BUNDLE" "$CONTEXTUAL_PROJECTION_BUNDLE" "$CONTEXT_BUNDLE" "$REQUEST_IR_BUNDLE" "$OPERATIONAL_BUNDLE" "$EMAIL_NEXUS_BUNDLE" "$CREDENTIAL_BUNDLE"' EXIT
...
node_modules/.bin/esbuild app/ava/operational-semantics.ts --bundle --platform=node --format=esm --outfile="$OPERATIONAL_BUNDLE" >/dev/null
...
DELENDA_AVA_OPERATIONAL_BUNDLE="file://$OPERATIONAL_BUNDLE" \
