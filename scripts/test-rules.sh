...
RESOLUTION_AUTHORITY_BUNDLE="$(mktemp "${TEMP_ROOT}/delenda-resolution-authority.XXXXXX.mjs")"
trap 'rm -f "$BUNDLE" "$CONVERGENCE_BUNDLE" "$APHORISM_BUNDLE" "$WAR_FEED_BUNDLE" "$ACCOUNT_TIME_BUNDLE" "$CAMPAIGN_PERSISTENCE_BUNDLE" "$RESOLUTION_AUTHORITY_BUNDLE"' EXIT
...
node_modules/.bin/esbuild app/resolution-authority.ts --bundle --platform=node --format=esm --outfile="$RESOLUTION_AUTHORITY_BUNDLE" >/dev/null
...
DELENDA_RESOLUTION_AUTHORITY_BUNDLE="file://$RESOLUTION_AUTHORITY_BUNDLE" node --test tests/resolution-authority.test.mjs
