...
WAR_FEED_BUNDLE="$(mktemp /tmp/delenda-war-feed.XXXXXX.mjs)"
ACCOUNT_TIME_BUNDLE="$(mktemp /tmp/delenda-account-time.XXXXXX.mjs)"
trap 'rm -f "$BUNDLE" "$CONVERGENCE_BUNDLE" "$APHORISM_BUNDLE" "$WAR_FEED_BUNDLE" "$ACCOUNT_TIME_BUNDLE"' EXIT
