...
RELEVANCE_BUNDLE="$(mktemp "${TEMP_ROOT}/delenda-ava-relevance.XXXXXX.mjs")"
trap 'rm -f "$BUNDLE" "$REPORTS_BUNDLE" "$GAME_BUNDLE" "$TERMINAL_BUNDLE" "$RUNTIME_BUNDLE" "$CONTEXT_BUNDLE" "$INTERFACE_BUNDLE" "$CHAT_EXPORT_BUNDLE" "$INTRUSION_LIBRARY_BUNDLE" "$INLINE_TOKENS_BUNDLE" "$RELEVANCE_BUNDLE"' EXIT
...
node_modules/.bin/esbuild app/ava/relevance-engine.ts --bundle --platform=node --format=esm --outfile="$RELEVANCE_BUNDLE" >/dev/null
...
DELENDA_AVA_RELEVANCE_BUNDLE="file://$RELEVANCE_BUNDLE" node --test tests/ava-relevance-engine.test.mjs
